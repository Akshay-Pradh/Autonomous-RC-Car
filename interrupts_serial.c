...
__interrupt void eUSCI_A0_ISR(void){
    ...
    switch(__even_in_range(UCA0IV,0x08)){
        ...
        case 2: // Vector 2 – RXIFG
            message_status = RECEIVED;          // change the message status to RECEIVED
            temp = iot_rx_ring_wr++;
        ...
        case 4: // Vector 4 – TXIFG
            UCA0TXBUF = transmit_array[t_index];    // transmit character
            if (transmit_array[t_index] == '\n') {  // check if newline, if so that was the last character to transmit
                ...
                message_status = TRANSMITTED;   // message status set to TRANSMITTED
            }
